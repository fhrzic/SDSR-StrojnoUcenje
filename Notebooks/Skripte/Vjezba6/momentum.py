...
def sgd_widget(
    ...
    """
    Tri panela (side-by-side):
      (Lijevo)  GD (full-batch) putanja na MSE konturama
      (Sredina) SGD (mini-batch) putanja na MSE konturama
      (Desno)   Nesterov momentum (mini-batch) putanja na MSE konturama

    Kontrole:
      θ₀, θ₁ (slideri + ručni unos), α (LR), iteracije, broj kontura,
      batch_size (SGD/Nesterov), seed (-1 = bez sjemena), β (momentum).
    """
    ...
    _theta0 = FloatSlider(value=1.0, min=-20.0, max=20.0, step=0.01,
                          description="θ₀", layout=Layout(width="200px"), continuous_update=True)
    ...
    _alpha  = FloatSlider(value=0.10, min=0.0, max=0.5, step=0.005,
                          description="α", layout=Layout(width="180px"), continuous_update=False)
    _iters  = IntSlider(value=20, min=0, max=300, step=1,
                        description="iter", layout=Layout(width="180px"), continuous_update=False)
    _levels = IntSlider(value=60, min=5, max=200, step=5,
                        description="konture", layout=Layout(width="200px"), continuous_update=False)

    _batch  = IntSlider(value=32, min=1, max=max(2, len(_x)), step=1,
                        description="batch", layout=Layout(width="180px"), continuous_update=False)
    _seed   = IntText(value=-1, description="seed", layout=Layout(width="120px"))  # -1 => None

    _beta   = FloatSlider(value=0.90, min=0.0, max=0.99, step=0.01,
                          description="β (mom.)", layout=Layout(width="200px"), continuous_update=False)

    _final_info_gd   = Label("Final GD: —")
    _final_info_sgd  = Label("Final SGD: —")
    _final_info_nest = Label("Final Nesterov: —")

    # --- Output paneli + figure handles ---
    _out_left   = Output(layout=Layout(border="1px solid #ddd", width="600px", height="560px"))
    _out_middle = Output(layout=Layout(border="1px solid #ddd", width="600px", height="560px"))
    _out_right  = Output(layout=Layout(border="1px solid #ddd", width="600px", height="560px"))
    _fig_left = _fig_mid = _fig_right = None

    # --- Precompute MSE konture (jednom) ---
    ...
    def _sgd_path():
        _th = np.array([float(_theta0.value), float(_theta1.value)], dtype=float)
        _thetas, _scores = [], []
        _rs = int(_seed.value); _seed_val = None if _rs < 0 else _rs
        for k in range(int(_iters.value)):
            _yhat_full = _th[0] + _th[1] * _x
            _scores.append(mse(_y, _yhat_full))
            _thetas.append(_th.copy())
            _g = sgd(y=_y, x=_x, theta=_th,
                     batch_size=int(_batch.value),
                     random_state=None if _seed_val is None else _seed_val + k)
            _th = _th - float(_alpha.value) * _g
        _yhat_full = _th[0] + _th[1] * _x
        _scores.append(mse(_y, _yhat_full))
        _thetas.append(_th.copy())
    ...
    def _nesterov_path():
        """
        Nesterov mini-batch momentum:
            m_{t+1} = β m_t + (1-β) * g( φ_t - αβ m_t )
            φ_{t+1} = φ_t - α m_{t+1}
        gdje g(·) računamo preko tvoje funkcije `sgd(...)` na lookahead točki.
        """
        _th = np.array([float(_theta0.value), float(_theta1.value)], dtype=float)
        _m = np.zeros(2, dtype=float)  # početni momentum
        _thetas, _scores = [], []
        _rs = int(_seed.value); _seed_val = None if _rs < 0 else _rs

        for k in range(int(_iters.value)):
            # evaluacija MSE u tren. točki radi vizualizacije
            _yhat_full = _th[0] + _th[1] * _x
            _scores.append(mse(_y, _yhat_full))
            _thetas.append(_th.copy())

            # lookahead: φ_t - αβ m_t
            _look = _th - float(_alpha.value) * float(_beta.value) * _m

            # gradijent na lookahead-u (mini-batch)
            _g_look = sgd(y=_y, x=_x, theta=_look,
                          batch_size=int(_batch.value),
                          random_state=None if _seed_val is None else _seed_val + k)

            # momentum i korak
            _m = float(_beta.value) * _m + (1.0 - float(_beta.value)) * _g_look
            _th = _th - float(_alpha.value) * _m
    ...
    # --- Crtanje sva tri panela ---
    def _draw_all():
        nonlocal _fig_left, _fig_mid, _fig_right
        ...
        _t0_nes, _t1_nes, _scores_nes, _theta_final_nes, _g_final_nes  = _nesterov_path()

        # LEFT: GD
        with _out_left:
            _out_left.clear_output(wait=True)
            if _fig_left is not None: plt.close(_fig_left)
            _fig_left, _axL = plt.subplots(figsize=(8.5, 6.2))
            try:
        ...
        with _out_middle:
            ...
            if _fig_mid is not None: plt.close(_fig_mid)
            _fig_mid, _axM = plt.subplots(figsize=(8.5, 6.2))
            try:
                _fig_mid.canvas.header_visible = False
                _fig_mid.canvas.toolbar_visible = False
            except Exception:
                pass
            _axM.contourf(_t0_mesh, _t1_mesh, _Z_vals, max(16, _levels.value * 4), cmap=_my_colormap)
            _axM.contour(_t0_mesh, _t1_mesh, _Z_vals, _levels.value, colors=['#80808080'])
            _axM.plot(_t0_sgd, _t1_sgd, 'bx-', linewidth=1.8, markersize=4, label=f"SGD putanja (batch={_batch.value})")
            _axM.set_xlabel(r'Offset $\phi_{0}$ (theta0)'); _axM.set_ylabel(r'Frequency $\phi_{1}$ (theta1)')
            _axM.set_title('Konture MSE + SGD (mini-batch)')
            _axM.legend(loc="best")
            plt.tight_layout(); plt.show()
        ...
        with _out_right:
            ...
            if _fig_right is not None: plt.close(_fig_right)
            _fig_right, _axR = plt.subplots(figsize=(8.5, 6.2))
            try:
            ...
            _axR.plot(_t0_nes, _t1_nes, 'r^-', linewidth=1.8, markersize=4, label=f"Nesterov (β={_beta.value:.2f})")
            _axR.set_xlabel(r'Offset $\phi_{0}$ (theta0)'); _axR.set_ylabel(r'Frequency $\phi_{1}$ (theta1)')
            _axR.set_title('Konture MSE + Nesterov momentum (mini-batch)')
            _axR.legend(loc="best")
            plt.tight_layout(); plt.show()
            _final_info_nest.value = (
                f"Final Nesterov θ: [θ₀={_theta_final_nes[0]:.4f}, θ₁={_theta_final_nes[1]:.4f}]   "
                f"∇MSE: [dθ₀={_g_final_nes[0]:.4e}, dθ₁={_g_final_nes[1]:.4e}]"
            )
    ...
    def _on_t0_slider(_chg):
        ...
        _is_syncing["t0"] = True; _theta0_txt.value = float(_theta0.value); _is_syncing["t0"] = False; _draw_all()
    def _on_t1_slider(_chg):
        if _is_syncing["t1"]: return
        _is_syncing["t1"] = True; _theta1_txt.value = float(_theta1.value); _is_syncing["t1"] = False; _draw_all()
    ...
    for _w in (_alpha, _iters, _levels, _batch, _seed, _beta):
        _w.observe(lambda _:_draw_all(), names="value")

   # --- Layout (tri grafa) ---
    # Controls split over two rows
    _controls_row1 = HBox([_theta0, _theta1, _theta0_txt, _theta1_txt],
                        layout=Layout(justify_content="flex-start", gap="10px"))
    _controls_row2 = HBox([_alpha, _iters, _levels, _batch, _seed, _beta],
                        layout=Layout(justify_content="flex-start", gap="10px"))

    # Two plots (left + middle) on row 3
    _plots_row1 = HBox([_out_left, _out_middle],
                    layout=Layout(justify_content="space-between", gap="16px"))

    # Third plot alone on row 4
    _plot_row4 = HBox([_out_right],
                    layout=Layout(justify_content="space-between", gap="16px"))

    # Info row (optional: move above/below as you prefer)
    _info_row = HBox([_final_info_gd, _final_info_sgd, _final_info_nest],
                    layout=Layout(justify_content="flex-start", gap="16px"))

    # Assemble: 1) controls row1, 2) controls row2, 3) two plots, 4) third plot, 5) info
    _ui = VBox([
        _controls_row1,     # row 1
        _controls_row2,     # row 2
        _plots_row1,        # row 3 (two plots)
        _plot_row4,         # row 4 (third plot)
        _info_row,          # row 5 (optional position)
    ])
