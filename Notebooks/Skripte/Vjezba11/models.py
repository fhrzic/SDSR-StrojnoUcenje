"""
Main class for deep MLP model on MNIST with selectable activation function,
various weight initialisation schemes and optional dropout.
"""

# Učitavanje knjižnica
from typing import Literal
import torchinfo
import torch
import torch.nn as nn


def print_model_summary(model: torch.nn.Module = None,
                        device: str = 'cpu',
                        input_dim=None):
    """
    Funkcija koja ispisuje sažetak modela.

    Argumenti:
        * model, pytorch model.
        * device, str, gdje se nalaze model i podaci, zadano je: cpu.
        * input_dim, tupple, ulazne dimenzije modela.

    Izlaz:
        * vraća ispisivi sažetak modela
    """
    return torchinfo.summary(
        model=model,
        input_size=input_dim,
        col_names=["input_size", "output_size", "num_params", "trainable"],
        col_width=16,
        device=device,
        row_settings=["var_names"]
    )


class DeepMLPNet(nn.Module):
    """
    Duboka potpuno povezana mreža (MLP) s:
      - odabirljivom aktivacijom (ReLU, sigmoid ili tanh)
      - odabirljivom inicijalizacijom težina:
          * 'xavier_uniform'
          * 'xavier_normal'
          * 'kaiming_uniform' (He)
          * 'zeros'          (sve težine = 0)
          * 'constant'       (sve težine = isti broj)
      - opcionalnim dropoutom u skrivenim slojevima

    Arhitektura:
        - Ulaz: spljoštena 28x28 MNIST slika (zadano)
        - `num_layers` skrivenih slojeva sa `hidden_dim` neurona
        - iza svakog skrivenog sloja:
            Linear -> aktivacija -> (opcijski Dropout)
        - Linearni izlazni sloj s `num_classes` neurona
    """

    def __init__(
        self,
        input_dim: int = 28 * 28,
        hidden_dim: int = 256,
        num_layers: int = 20,
        num_classes: int = 10,
        init_scale: float = 1.0,
        activation: Literal["relu", "sigmoid", "tanh"] = "sigmoid",
        init_type: Literal[
            "xavier_uniform",
            "xavier_normal",
            "kaiming_uniform",
            "zeros",
            "constant",
        ] = "xavier_uniform",
        init_constant_value: float = 0.01,
        dropout_prob: float = 0.0,
    ):
        super().__init__()

        self.init_scale = init_scale
        self.activation = activation.lower()
        self.init_type = init_type.lower()
        self.init_constant_value = init_constant_value
        self.dropout_prob = float(dropout_prob)

        assert self.activation in ["relu", "sigmoid", "tanh"], \
            f"activation mora biti 'relu', 'sigmoid' ili 'tanh', dobiveno: {activation}"

        assert self.init_type in [
            "xavier_uniform",
            "xavier_normal",
            "kaiming_uniform",
            "zeros",
            "constant",
        ], (
            "init_type mora biti jedan od: "
            "'xavier_uniform', 'xavier_normal', 'kaiming_uniform', 'zeros', 'constant' "
            f"(dobiveno: {init_type})"
        )

        assert 0.0 <= self.dropout_prob < 1.0, \
            f"dropout_prob mora biti u [0, 1), dobiveno: {dropout_prob}"

        _layers = []
        _last_dim = input_dim

        for _ in range(num_layers):
            _fc = nn.Linear(_last_dim, hidden_dim)
            self._init_weights(_fc)
            _layers.append(_fc)
            _last_dim = hidden_dim

        self.hidden_layers = nn.ModuleList(_layers)

        self.output_layer = nn.Linear(_last_dim, num_classes)
        self._init_weights(self.output_layer)

        # Dropout modul (ako je p=0, Dropout se efektivno ne primjenjuje)
        if self.dropout_prob > 0.0:
            self.dropout = nn.Dropout(p=self.dropout_prob)
        else:
            self.dropout = nn.Identity()

    def _init_weights(self, _layer: nn.Linear) -> None:
        """
        Pomoćna metoda za inicijalizaciju težina jednog Linear sloja
        prema odabranom `init_type` i `init_scale`.
        """
        if self.init_type == "xavier_uniform":
            nn.init.xavier_uniform_(_layer.weight)
            nn.init.zeros_(_layer.bias)
        elif self.init_type == "xavier_normal":
            nn.init.xavier_normal_(_layer.weight)
            nn.init.zeros_(_layer.bias)
        elif self.init_type == "kaiming_uniform":
            nn.init.kaiming_uniform_(
                _layer.weight,
                nonlinearity="relu",
            )
            nn.init.zeros_(_layer.bias)
        elif self.init_type == "zeros":
            nn.init.zeros_(_layer.weight)
            nn.init.zeros_(_layer.bias)
        elif self.init_type == "constant":
            nn.init.constant_(_layer.weight, self.init_constant_value)
            nn.init.zeros_(_layer.bias)
        else:
            raise ValueError(f"Nepodržan init_type: {self.init_type}")

        if self.init_type not in ["zeros", "constant"]:
            _layer.weight.data *= self.init_scale

    def _act_fn(self, _x: torch.Tensor) -> torch.Tensor:
        if self.activation == "sigmoid":
            return torch.sigmoid(_x)
        elif self.activation == "relu":
            return torch.relu(_x)
        elif self.activation == "tanh":
            return torch.tanh(_x)
        raise ValueError(f"Nepodržana aktivacija: {self.activation}")

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """
        Standardni forward prolaz kroz mrežu:
        - spljoštavanje slike ako je potrebno
        - prolazak kroz skrivene slojeve + aktivacija + (opcijski dropout)
        - izlazni linearni sloj bez aktivacije (logiti)
        """
        if x.dim() > 2:
            _x = x.view(x.size(0), -1)
        else:
            _x = x

        for _layer in self.hidden_layers:
            _x = self._act_fn(_layer(_x))
            _x = self.dropout(_x)

        _x = self.output_layer(_x)
        return _x


class SimpleConv2DNet(nn.Module):
    """
    Jednostavna 2D CNN mreža za klasifikaciju MNIST slika.

    Ulaz:
        x oblika (B, 1, H, W), npr. (B, 1, 28, 28)

    Parametri (po želji ih možeš mijenjati u widgetu ili ovdje u kodu):
        - num_layers: broj Conv2d slojeva
        - filters_per_layer: broj filtera po sloju
        - kernel_size: veličina jezgre (3, 5, 7 → 3x3, 5x5, 7x7)
    """

    def __init__(
        self,
        in_channels: int = 1,
        num_classes: int = 10,
        num_layers: int = 3,
        filters_per_layer: int = 16,
        kernel_size: int = 3,
        input_height: int = 28,
        input_width: int = 28,
    ):
        super().__init__()

        assert kernel_size % 2 == 1, "kernel_size bi trebao biti neparan (3, 5, 7, ...)"

        self.in_channels = in_channels
        self.num_classes = num_classes
        self.num_layers = num_layers
        self.filters_per_layer = filters_per_layer
        self.kernel_size = kernel_size
        self.input_height = input_height
        self.input_width = input_width

        padding = kernel_size // 2  # da zadržimo H,W dimenzije

        conv_layers = []
        last_ch = in_channels
        for _ in range(num_layers):
            conv = nn.Conv2d(
                in_channels=last_ch,
                out_channels=filters_per_layer,
                kernel_size=kernel_size,
                padding=padding,
            )
            # Kaiming (He) inicijalizacija za ReLU
            nn.init.kaiming_uniform_(conv.weight, nonlinearity="relu")
            nn.init.zeros_(conv.bias)

            conv_layers.append(conv)
            last_ch = filters_per_layer

        self.convs = nn.ModuleList(conv_layers)

        # Izračun flatten dimenzije preko dummy prolaza
        with torch.no_grad():
            dummy = torch.zeros(1, in_channels, input_height, input_width)
            _x = dummy
            for conv in self.convs:
                _x = torch.relu(conv(_x))
            flattened_dim = _x.view(1, -1).shape[1]

        self.fc = nn.Linear(flattened_dim, num_classes)
        nn.init.kaiming_uniform_(self.fc.weight, nonlinearity="relu")
        nn.init.zeros_(self.fc.bias)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """
        x: (B, C=1, H, W), npr. (B, 1, 28, 28)
        """
        _x = x
        for conv in self.convs:
            _x = torch.relu(conv(_x))

        _x = _x.view(_x.size(0), -1)
        _x = self.fc(_x)
        return _x


# ======================================================================
# ResNet-like MLP (BatchNorm + ReLU residual blocks)
# ======================================================================

class ResidualBNReLUBlock(nn.Module):
    """Pre-activation rezidualni blok (ResNet v2 stil):

    BN -> ReLU -> Linear -> BN -> ReLU -> Linear -> (+ skip)

    Napomena: nema završne ReLU nakon zbrajanja; sljedeći blok započinje s BN+ReLU.
    Pretpostavlja konstantnu dimenziju vektora (`hidden_dim`) kroz cijelu mrežu.
    """

    def __init__(self, hidden_dim: int):
        super().__init__()
        self.bn1 = nn.BatchNorm1d(hidden_dim)
        self.fc1 = nn.Linear(hidden_dim, hidden_dim, bias=False)
        self.bn2 = nn.BatchNorm1d(hidden_dim)
        self.fc2 = nn.Linear(hidden_dim, hidden_dim, bias=False)

        # Kaiming inicijalizacija za ReLU (linear slojevi u blokovima)
        nn.init.kaiming_uniform_(self.fc1.weight, nonlinearity="relu")
        nn.init.kaiming_uniform_(self.fc2.weight, nonlinearity="relu")

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        identity = x

        out = self.bn1(x)
        out = torch.relu(out)
        out = self.fc1(out)

        out = self.bn2(out)
        out = torch.relu(out)
        out = self.fc2(out)

        out = out + identity
        return out


class ResidualBNReLUMLPNet(nn.Module):
    """MLP s pre-activation rezidualnim blokovima (ResNet-like) i BatchNorm-om.

    Arhitektura (za MNIST):
      - stem: Linear(input_dim -> hidden_dim)
      - num_blocks x [PreAct residual block]
      - završetak: BN -> ReLU -> Linear(hidden_dim -> num_classes)

    Ulaz može biti oblika (B, 1, 28, 28) ili (B, input_dim).
    """

    def __init__(
        self,
        hidden_dim: int = 256,
        num_blocks: int = 4,
        num_classes: int = 10,
        input_dim: int = 28 * 28,
    ):
        super().__init__()
        assert num_blocks >= 1, "num_blocks mora biti >= 1"
        self.input_dim = input_dim
        self.hidden_dim = hidden_dim
        self.num_blocks = num_blocks
        self.num_classes = num_classes

        self.stem = nn.Linear(input_dim, hidden_dim, bias=False)
        nn.init.kaiming_uniform_(self.stem.weight, nonlinearity="relu")

        self.blocks = nn.ModuleList([ResidualBNReLUBlock(hidden_dim) for _ in range(num_blocks)])

        self.final_bn = nn.BatchNorm1d(hidden_dim)
        self.classifier = nn.Linear(hidden_dim, num_classes)
        # Kaiming inicijalizacija je razumna i za završni linear (ulaz je nakon ReLU)
        nn.init.kaiming_uniform_(self.classifier.weight, nonlinearity="relu")
        if self.classifier.bias is not None:
            nn.init.zeros_(self.classifier.bias)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        if x.dim() == 4:
            x = x.view(x.size(0), -1)

        out = self.stem(x)

        for blk in self.blocks:
            out = blk(out)

        out = self.final_bn(out)
        out = torch.relu(out)
        out = self.classifier(out)
        return out
