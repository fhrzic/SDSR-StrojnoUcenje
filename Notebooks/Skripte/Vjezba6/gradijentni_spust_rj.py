...
def gradijent_aproksimacija(y: np.ndarray = None,
    ...
    if _theta.size != 2:
        raise ValueError("_theta mora imati 2 elementa: [theta0, theta1].")
    if delta <= 0:
        raise ValueError("_delta mora biti > 0.")

    _theta0, _theta1 = float(theta[0]), float(theta[1])

    # --- bazni MSE: L(theta0, theta1) ---
    _y_hat = _theta0 + _theta1 * _x
    _L0 = mse(_y, _y_hat)

    # --- d/dtheta0 (forward diff) ---
    _y_hat_d0 = (_theta0 + delta) + _theta1 * _x
    _Ld0 = mse(_y, _y_hat_d0)
    _dtheta0 = (_Ld0 - _L0) / delta

    # --- d/dtheta1 (forward diff) ---
    _y_hat_d1 = _theta0 + (_theta1 + delta) * _x
    _Ld1 = mse(_y, _y_hat_d1)
    _dtheta1 = (_Ld1 - _L0) / delta
