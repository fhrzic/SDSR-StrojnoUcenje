...
from models import DeepMLPNet, SimpleConv2DNet, ResidualBNReLUMLPNet, print_model_summary
...
def _compute_macro_f1(y_true: torch.Tensor, y_pred: torch.Tensor, num_classes: int | None = None) -> float:
    """
...
def _evaluate_macro_f1(
    ...
    macro_f1 = _compute_macro_f1(all_targets, all_preds, num_classes=10)
...
def resnet_mlp_simple_widget():
    """
    Widget za treniranje ResNet-like MLP modela (pre-activation BN+ReLU rezidualni blokovi) na MNIST-u.

    Kontrole:
      - learning rate, broj epoha, batch size
      - augmentacija (DA/NE)
      - hidden_dim (dimenzija skrivenog vektora)
      - num_blocks (broj rezidualnih blokova)
    """

    output = Output(layout=Layout(border="1px solid #ccc"))

    # ---- Kontrole (stilom slično ostalima) ----
    lr_slider = widgets.FloatLogSlider(
        description="LR",
        value=1e-3,
        base=10,
    ...
    train_button = widgets.Button(
        description="Treniraj ResNet-MLP",
        button_style="success",
        icon="play",
        layout=Layout(width="220px", height="40px"),
    )
    ...
    def _on_train_clicked(_b):
        with output:
            ...
            num_epochs = int(epochs_slider.value)
            hidden_dim = int(hidden_dim_slider.value)
            num_blocks = int(blocks_slider.value)
            ...
            # ResNet-like MLP model (pre-activation residual blocks)
            model = ResidualBNReLUMLPNet(
                input_dim=28 * 28,
                hidden_dim=hidden_dim,
            ...
            print("\n=== Model summary (ResNet-MLP) ===")
            summary_obj = print_model_summary(
                model=model.to(device),
                device=str(device),
                input_dim=(1, 28 * 28),   # MLP-style summary input
            )
            print(summary_obj)

            print("\n=== Pokrećem treniranje ResNet-MLP mreže ===")
            _ = _train_with_macro_f1(
                model=model,
                train_loader=train_loader,
                valid_loader=valid_loader,
                ...
                num_epochs=num_epochs,
                learning_rate=lr,
                device=device,  # torch.device, consistent with other widgets
            )

            print("\nTrening ResNet-MLP mreže završen.")

    train_button.on_click(_on_train_clicked)

    # Layout: slično kao MLP/Conv2D widget
    row1 = HBox([lr_slider, epochs_slider, batch_slider], layout=Layout(gap="10px"))
    row2 = HBox([hidden_dim_slider, blocks_slider, augment_checkbox], layout=Layout(gap="10px"))
    row3 = HBox([train_button], layout=Layout(gap="10px"))
...
def unified_mlp_conv2d_widget():
    ...
    mlp_out    = mlp_simple_widget()
    conv_out   = conv2d_simple_widget()
    resnet_out = resnet_mlp_simple_widget()   

    tab.children = [mlp_out, conv_out, resnet_out]

    tab.set_title(0, "MLP")
    tab.set_title(1, "Conv2D")
    tab.set_title(2, "ResNet-MLP")             
